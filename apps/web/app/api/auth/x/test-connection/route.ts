...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    recommendations: [
      "1. X Developer Portal → Keys and tokens → OAuth 2.0 Client ID → Copy the full value (typically 40+ characters)",
      "2. Vercel → Environment Variables → X_CLIENT_ID → Paste (no spaces or new lines)",
      "3. X Developer Portal → Settings → User authentication settings → Confirm the callback URI:",
      "   MUST BE EXACTLY: https://aura-creatures.vercel.app/api/auth/x/callback",
      "   - Use https:// (not http)",
      "   - Must not end with /",
      "   - Case sensitive",
      "4. App permissions: Read (not Read and write)",
      "5. Type of App: Web App, Automated App or Bot (not Native App)",
      "6. OAuth 2.0 Settings → Must be enabled",
      "7. App status: Must be Active (not suspended/pending)",
      "8. After making changes: Save → wait 2-3 minutes → redeploy",
    ],
    troubleshooting: {
      step1: "Open the browser console (F12) and click the 'Connect X Account' button",
      step2: "Inspect the client_id parameter in the authorization URL",
      step3: "Compare it with X Developer Portal → Keys and tokens → Client ID",
      step4: "If it doesn't match → update the Vercel environment variable",
      step5: "X Portal → Analytics → User authentication → Review error logs",
    },
  });
}

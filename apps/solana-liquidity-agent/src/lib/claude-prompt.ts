...
**RISK SCORING GUIDELINES - CRITICAL: USE FULL RANGE (0-100), NOT JUST 70-80!**

**⚠️ CRITICAL WARNING: You MUST use the FULL range (0-100). Do NOT cluster scores around 70-80. Each token is unique - score accordingly!**

You must calculate the risk score based on ALL the detailed information in your analysis, NOT just basic metrics. Consider:

**VERY LOW RISK (0-25):** Score very low when you see EXCEPTIONAL safety:
- Very high diamond hands ratio (>75% early buyers still holding)
- Minimal manipulation (<3% wash trading)
- Excellent wallet distribution (top wallet <15% of volume)
- Strong early buyer conviction (most in profit, holding long-term)
- Very low profit-taking risk
- Strong support levels with high volume
- Organic, diverse new wallet growth
- Healthy trading patterns (balanced buy/sell, normal velocity)
- No dangerous authorities (no freeze/mint)
- Example: "75% diamond hands, 2% wash trading, top wallet 12%" → Score: 15-25

**LOW RISK (26-40):** Score low when you see GOOD safety:
- High diamond hands ratio (60-75% early buyers still holding)
- Low manipulation patterns (3-5% wash trading)
- Healthy wallet distribution (top wallet 15-20% of volume)
...
- Example: "65% diamond hands, 4% wash trading, top wallet 18%" → Score: 30-40

**MEDIUM RISK (41-60):** Score medium when you see MODERATE concerns:
- Moderate diamond hands (40-60% early buyers still holding)
...
- Example: "50% diamond hands, 7% wash trading, top wallet 25%" → Score: 50-60

**HIGH RISK (61-75):** Score high when you see SIGNIFICANT concerns:
- Low diamond hands (20-40% early buyers still holding)
- Significant manipulation patterns (10-15% wash trading, coordinated activity)
- High wallet concentration (30-40% top wallet)
- Many early buyers selling
...
- Weak support levels
- Example: "30% diamond hands, 12% wash trading, top wallet 35%" → Score: 65-75

**VERY HIGH RISK (76-90):** Score very high when you see SEVERE red flags:
- Very low diamond hands (<20% early buyers holding)
- Severe manipulation (>15% wash trading, clear coordinated patterns)
- Critical wallet concentration (40-50% top wallet)
- Extreme profit-taking risk
- Clear manipulation signs (freeze/mint authority + manipulation)
- Fake activity (bot farm with identical patterns)
- Example: "15% diamond hands, 18% wash trading, top wallet 45%" → Score: 80-90

**CRITICAL RISK (91-100):** Score critical ONLY when you see EXTREME danger:
- Almost no diamond hands (<10% early buyers holding)
- Extreme manipulation (>20% wash trading, obvious coordinated patterns)
- Extreme wallet concentration (>50% top wallet)
- Clear rug pull signs (freeze/mint authority + extreme manipulation)
- Dead pool (no real trading activity, all fake)
- Example: "5% diamond hands, 25% wash trading, top wallet 60%, freeze authority enabled" → Score: 95-100

**CRITICAL SCORING RULES - READ CAREFULLY:**
1. **USE THE FULL RANGE** - Don't default to 70-80! If a token is genuinely safe (high diamond hands, low manipulation), score it 20-40. If it's genuinely dangerous, score it 85-100.
2. **Weight the factors correctly:**
   - Diamond hands % is VERY important (60%+ = lower score, <30% = higher score)
   - Manipulation % is CRITICAL (>10% = higher score, <5% = lower score)
   - Wallet concentration matters (top wallet >30% = higher score)
   - Early buyer behavior is important (most holding = lower score, most selling = higher score)
3. **Calculate a weighted score:**
   - Start with a base score of 50 (medium)
   - If diamond hands >70%: subtract 25-30 points
   - If diamond hands 50-70%: subtract 10-20 points
   - If diamond hands <30%: add 20-30 points
   - If wash trading <5%: subtract 10-15 points
   - If wash trading >10%: add 15-25 points
   - If top wallet <20%: subtract 5-10 points
   - If top wallet >30%: add 10-20 points
   - If freeze/mint authority: add 15-25 points
   - Then adjust based on other factors
4. **Be specific and accurate:**
   - "70% diamond hands, 3% wash trading, top wallet 15%" → This is LOW RISK, score 25-35, NOT 70!
   - "25% diamond hands, 15% wash trading, top wallet 40%" → This is HIGH RISK, score 75-85
   - "50% diamond hands, 8% wash trading, top wallet 25%" → This is MEDIUM RISK, score 50-60
5. **Avoid clustering** - If you find yourself scoring 70-78 for most tokens, you're doing it wrong! Use the full range based on actual data.
6. **Reward good behavior** - High diamond hands (60%+), low manipulation (<5%), good distribution (top wallet <20%) = LOW RISK (20-40)
7. **Penalize bad behavior** - Low diamond hands (<30%), high manipulation (>10%), high concentration (top wallet >30%) = HIGH RISK (70-90)

**SCORING EXAMPLES:**
- Token A: 80% diamond hands, 2% wash trading, top wallet 10%, no dangerous authorities → **Score: 15-25** (VERY LOW RISK)
- Token B: 65% diamond hands, 4% wash trading, top wallet 18%, no dangerous authorities → **Score: 30-40** (LOW RISK)
- Token C: 45% diamond hands, 8% wash trading, top wallet 28%, no dangerous authorities → **Score: 55-65** (MEDIUM-HIGH RISK)
- Token D: 20% diamond hands, 15% wash trading, top wallet 45%, freeze authority enabled → **Score: 85-95** (VERY HIGH RISK)
- Token E: 5% diamond hands, 25% wash trading, top wallet 65%, freeze+mint authority → **Score: 95-100** (CRITICAL RISK)
...
**CALCULATE RISK SCORE BASED ON YOUR ANALYSIS - STEP BY STEP:**

**STEP 1: After writing your complete analysis, identify these key metrics:**
- Diamond hands percentage (early buyers still holding) - MOST IMPORTANT
- Wash trading percentage - CRITICAL
- Top wallet concentration % - IMPORTANT
- Early buyer profit/loss status - IMPORTANT
- Freeze/mint authority status - CRITICAL if present
...
**STEP 2: Calculate a weighted score using this formula:**
1. Start with base score: 50 (medium risk)
2. Diamond hands adjustment:
   - If >70%: subtract 25-30 points
   - If 50-70%: subtract 10-20 points
   - If 30-50%: no change
   - If <30%: add 20-30 points
3. Wash trading adjustment:
   - If <3%: subtract 10-15 points
   - If 3-5%: subtract 5-10 points
   - If 5-10%: no change
   - If >10%: add 15-25 points
4. Wallet concentration adjustment:
   - If top wallet <15%: subtract 5-10 points
   - If top wallet 15-25%: no change
   - If top wallet >30%: add 10-20 points
5. Authority adjustment:
   - If freeze authority: add 15-20 points
   - If mint authority: add 10-15 points
   - If both: add 25-30 points
6. Final adjustments based on other factors (profit-taking risk, support levels, etc.)

**STEP 3: Use the full range (0-100) - DO NOT cluster around 70-80!**
- If calculated score is 20-30, use it! Don't round up to 70.
- If calculated score is 85-95, use it! Don't round down to 75.
- Each token is unique - score it accurately based on the data.

**Scoring Ranges with Examples:**
- **0-25:** Very Safe - 75%+ diamond hands, <3% wash trading, top wallet <15% → **Example: Score 20**
- **26-40:** Mostly Safe - 60-75% diamond hands, 3-5% wash trading, top wallet 15-20% → **Example: Score 35**
- **41-60:** Be Careful - 40-60% diamond hands, 5-10% wash trading, top wallet 20-30% → **Example: Score 55**
- **61-75:** Dangerous - 20-40% diamond hands, 10-15% wash trading, top wallet 30-40% → **Example: Score 70**
- **76-90:** Very Dangerous - <20% diamond hands, >15% wash trading, top wallet >40% → **Example: Score 85**
- **91-100:** Critical - <10% diamond hands, >20% wash trading, top wallet >50%, dangerous authorities → **Example: Score 95**
